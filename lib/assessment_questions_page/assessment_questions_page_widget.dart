    ...
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      ...
      if ((_model.assessmentLangProfile?.succeeded ?? true)) {
        ...
        if ((_model.loadedAssessmentCopy?.succeeded ?? true)) {
          ...
          _model.loadedAssessmentCopyRetry =
          ...
          if ((_model.loadedAssessmentCopyRetry?.succeeded ?? true)) {
            ...
            safeSetState(() {});
            _model.q1Label = AssessmentQuestionsFlatResponseStruct.maybeFromMap(
                    (_model.loadedAssessmentCopyRetry?.jsonBody ?? ''))
                ?.q1Label;
            safeSetState(() {});
            _model.q1A = AssessmentQuestionsFlatResponseStruct.maybeFromMap(
                    (_model.loadedAssessmentCopyRetry?.jsonBody ?? ''))
            ...
            safeSetState(() {});
            _model.q10Label =
                AssessmentQuestionsFlatResponseStruct.maybeFromMap(
                        (_model.loadedAssessmentCopyRetry?.jsonBody ?? ''))
                    ?.q10Label;
            safeSetState(() {});
            _model.q10A = AssessmentQuestionsFlatResponseStruct.maybeFromMap(
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  'Could not load the questions. Check your connection, then reopen this page.',
                  style: TextStyle(),
                ),
                              ...
                              if (_model.ans10 == '') {
                                ...
                                if ((_model.submitAssessmentResult?.succeeded ??
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                      content: Text(
                                        'Some answers are missing. Use Back to check every question, then try again.',
                                        style: TextStyle(),
                                      ),
                                      duration: Duration(milliseconds: 4000),
                                    ),
